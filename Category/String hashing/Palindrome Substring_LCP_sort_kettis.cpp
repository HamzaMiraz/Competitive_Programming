...
int lcp(int i,int len1,int x,int len2){
...
bool com(array<ll,3>a,array<ll,3>b){
    int ans=lcp(a[0],a[1],b[0],b[1]);
    if(ans==min(a[1],b[1])){
        return a[1]<b[1];
    }
    if(s[a[0]+ans]<=s[b[0]+ans]){
        return true;
    }else{
        return false;
    }
...
int main() {
    ...
    while(cin>>s){
        ...
        int ln=s.size();
        for(int l=2;l<=ln;l++){
            for(int i=0;i+l-1<ln;i++){
                if(IsPallindrome(i,i+l-1)){
                    v.push_back({i,l,substring_hash(i,i+l-1)});
                }
            }
        }
