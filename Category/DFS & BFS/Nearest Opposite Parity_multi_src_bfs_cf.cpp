...
void multi_source__bfs(vector<int>&od,vector<int>&ev,vector<int>&ans){
    vector<int>dis(n+2,oo);
    queue<int>q;
    for(auto it : od){
        dis[it]=0;
        q.push(it);
...
//    ios_base::sync_with_stdio(0); cin.tie(0);
    while(cin>>n)
    {
        vector<int>ans(n+2,oo);
        vector<int>od,ev;
        for(int i=1;i<=n;i++){
            cin>>ar[i];
            if(ar[i]%2==0)ev.push_back(i);
            else od.push_back(i);

            if(i+ar[i]<=n)adj[i+ar[i]].push_back(i);
            if(i-ar[i]>=1)adj[i-ar[i]].push_back(i);
        }
        multi_source__bfs(od,ev,ans);
        multi_source__bfs(ev,od,ans);
        for(int i=1;i<=n;i++){
            if(ans[i]==oo)ans[i]=-1;
            cout<<ans[i]<<" ";
        }
