...
int n,q;
int d,m;
ll go(int i,int take,int sum,vector<vector<vector<ll>>>&dp){
    if(i==n){
        if(take==m)return (sum==0);
        return 0;
    }
    if(take==m)return (sum==0);
    if(dp[i][take][sum]!=-1)return dp[i][take][sum];

    return dp[i][take][sum]=go(i+1,take+1,(sum+(ar[i]%d)+d)%d,dp)+go(i+1,take,sum,dp);
}
int ks;
int main()
{
...
//    ios_base::sync_with_stdio(0); cin.tie(0);
    ...
    while(t--)
    {
        ...
        while(q--){
            cin>>d>>m;
            vector<vector<vector<ll>>>dp(n+1,vector<vector<ll>>(m+1,vector<ll>(21,-1)));
            cout<<go(0,0,0,dp)<<"\n";
