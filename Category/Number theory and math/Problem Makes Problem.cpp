...
template <typename T>
T BIGMOD(T base,T power,T Mod=2e9+7){T ans = 1;
while(power){if(power & 1)ans = (ans * base) % Mod;
base = (base * base) % Mod;power = power >> 1;}return ans;}

template <typename T>
T INV(T base,T mod=2e9+7){//defult mod=1e9+7
...
//    ios_base::sync_with_stdio(0); cin.tie(0);
    ...
    while(t--)
    {
        ...
        /**
          suppose n=8,k=3;
          imagine 8 candy distritute among 3 children
          the scenario looks like **|****|**  , ***|****|* , *******|*| ,....
            (8+2)!/8!2! WAYS    

         **/
        ll ans=((f[n+k-1]%MOD)*INV(f[n]*f[k-1],MOD))%MOD;
        cout<<"Case "<<++ks<<": "<<ans;
        el
