...
#define F  first
#define S  second
const int MAX = 1e3 + 10;// string max size
const ll MOD1 = 1e9 + 7;
const ll MOD2 = 1e9 + 9;
const ll base1 = 269;//31,//53
const ll base2 = 277;//31,//53
const ll oo = 1e14;//31,//53
...
nd  compute_prehash(string const &s){///O(string size)
    ...
    ll sz=s.size();
    return {(hash_value.F*MOD2+hash_value.S) ,sz};
}
...
ll go(int len,int lst=3001){
    ...
    for(int i=0;i<n;i++){
        if(i==lst)continue;
        if(len+st[i].ln-1<txt.size() && substring_hash(len,len+st[i].ln-1)==st[i].hs){
            rf=min(rf,go(len+st[i].ln,i)+1);
        }
    }
    return rf;
}
...
///https://codeforces.com/gym/104763/problem/G