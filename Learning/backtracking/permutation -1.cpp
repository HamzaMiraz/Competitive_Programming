...
void go(int d, int cnt)
{
    ...
    for(int i=1;i<=n;i++)
        if(color[i])
            cout<<i<<", " ;///konta konta newa baki ase
    cout<<endl;

    for(int i=1;i<=n;i++){
        cout<<"fun:"<<cnt<<" LOOP CYCLE:"<<i<<endl;
        if(color[i])
        {
            sto[d] = i;
            ...
            color[i] = false;
            debug(i, "Start");
           // printf("function number = %d Start\n",cnt);
            deb(d);
            go(d+1, cnt+1);
            printf("function: %d LoopCycle: %d End\n",cnt,i);
            deb(d);
            debug(i, "End");
            color[i] = true;
        }
