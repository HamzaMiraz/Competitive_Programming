...
void solve(){
    ...
    int sl[n+2];
    vector<vector<ll>>dist(n+2,vector<ll>(1005,1e12));
    for(int i=1;i<=n;i++){
        cin>>sl[i];
    }
    minHeap<array<ll,4>>mh;
    mh.push({0,1000,1});
    dist[1][1000]=0;
    ll ans=1e12;
    while(!mh.empty()){
        array<ll,4> it=mh.top();
        ll u=it[2];
        ll dis=it[0];
        ll sln=it[1];
        mh.pop();
        ...
        for(int i=0;i<adj[u].size();i++){
            ...
            ll newsl=min(sln,1LL*sl[u]);
            ll newdis=dis+((1LL*w)*(newsl));
            if(dist[v][newsl]>newdis){
                dist[v][newsl]=newdis;
                mh.push({newdis,newsl,v});
            }
        }
    }
    cout<<ans<<"\n";
