...
int v[100005];
ll w[100005];
ll go(int i,int total_value){
    if(total_value<0)return 1e10;
    if(i==n){
        if(total_value==0)return 0;
        return 1e10;
    }
    ll &rf=dp[i][total_value];
    if(rf!=-1)return rf;
    return rf=min(go(i+1,total_value-v[i])+w[i],go(i+1,total_value));
}
...
//    ios_base::sync_with_stdio(0); cin.tie(0);
    ...
    while(cin>>n>>capacity)
    {
        ...
        int maxcarry=0;
        for(int val=0;val<=total_value;val++){
            if(go(0,val)<=capacity){
                maxcarry=val;
            }
        }
        cout<<maxcarry<<"\n";
