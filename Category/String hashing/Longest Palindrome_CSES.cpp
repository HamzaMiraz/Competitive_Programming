...
void Longest_Palindrome(string const &s){
    hashing(s);
    int n=s.size();
    int ans=1;
    int b=0,e=0;
    for(int i=0;i<n;i++){// for odd length
        ...
        while(l<=r){
            int m = l+r >> 1;
            if(GetPrefixHash(i-m+1,i)==GetSuffixHash(i,i+m-1)){
                if(ans < (2*m -1)){
                    ans=(2*m -1);
                    b=i-m+1;
                    e=i+m-1;
    ...
    for(int i=0;i<n;i++){// for even length
        if(i+1<n && s[i]==s[i+1]){
            int r=min(i+1,n-i-1);
            int l=1;
            while(l<=r){
                int m = l+r >> 1;
                if(GetPrefixHash(i-m+1,i)==GetSuffixHash(i+1,i+1+m-1)){
                    if(ans < (2*m )){
                        ans=(2*m );
                        b=i-m+1;
                        e=i+1+m-1;
