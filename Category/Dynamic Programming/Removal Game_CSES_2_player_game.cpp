...
ll ar[5005];
ll dp[5005][5005][4];
bool vis[5005][5005][4];
ll go(int i,int j,int lst){
    if(i>j)return 0;
    
    if(vis[i][j][lst])return dp[i][j][lst];
    vis[i][j][lst]=true;
    if(lst==2){
        dp[i][j][lst]=max(go(i+1,j,1)+ar[i],go(i,j-1,1)+ar[j]);
    }else{
        dp[i][j][lst]=min(go(i+1,j,2),go(i,j-1,2));
    }
    return dp[i][j][lst];
}
